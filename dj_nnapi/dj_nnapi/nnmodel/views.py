...
class PredictAll(APIView):
    ...
    def post(self, request: Request, *args, **kwargs):
        ...
        return Response(data={'text':'predicted', 'k': k}, status=status.HTTP_201_CREATED)
    ...
    def predict(self, file_path: str, projection_type: str, id: int):
        ...
        details = pre_details if track is None else segmetationDataForm(list(track.values())[0])
        ...
        for j,mask_img in enumerate(nn_mask):
          c,h = cv.findContours(mask_img,cv.RETR_TREE, cv.CHAIN_APPROX_SIMPLE | cv.CHAIN_APPROX_TC89_L1)
          # print(c, h)
          if h is not None:
